...
function Icon({ name }) {
  const paths = {
    ...
    help: <><circle cx="12" cy="12" r="9" /><path d="M9.7 9a2.4 2.4 0 1 1 4.3 1.5c-.9 1.1-2 1.3-2 3M12 17h.01" /></>,
...
export default function DashboardSidebar({ active = "Browse Tutors" }) {
  ...
  return (
    <aside className="dashboard-sidebar" aria-label="Student dashboard navigation">
      <nav className="sidebar-nav">
        ...
        <button className="sidebar-link sidebar-logout" type="button" onClick={() => navigate("/")}>
          <Icon name="logout" />
          <span>Logout</span>
        </button>
        <div className="become-tutor-card">
          ...
          <p>Share your knowledge and help students achieve their goals.</p>
          <button type="button" onClick={() => navigate("/register")}>Join as Tutor</button>
        </div>
      </nav>
    </aside>
  );
